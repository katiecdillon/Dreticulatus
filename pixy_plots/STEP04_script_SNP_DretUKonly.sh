...
wd='/scratch/kcd88651/ticks/D_reticulatus/pixy/DretUKonly'
...
bcftools mpileup --threads 32 -f "$DretUK_a" $wd/DretUK.DretUK.sorted.bam -r "$chr" \
    | bcftools call --threads 32 -m -O z -a GQ -o chr${numb}.DretUKonly.vcf.gz

echo "Done: chr${numb}.DretUKonly.vcf.gz"
